...
// ugc is the HTML sanitization policy used for post content.
// It starts from bluemonday's UGC policy and additionally permits
// a curated set of inline CSS properties on allowed elements.
var ugc = newUGCPolicy()

// allowedCSSProperties lists CSS properties that may appear in style attributes.
// Values are still validated by bluemonday's default CSS handlers so that
// dangerous constructs (for example javascript: URLs) are rejected.
var allowedCSSProperties = []string{
	...
	"display", "empty-cells", "filter",
	"flex", "flex-basis", "flex-direction", "flex-flow", "flex-grow",
	"flex-shrink", "flex-wrap", "float",
	"font", "font-family", "font-kerning", "font-language-override",
	"font-size", "font-size-adjust", "font-stretch", "font-style",
	"font-synthesis", "font-variant", "font-variant-caps",
...
// newUGCPolicy builds the sanitization policy for user-generated post HTML.
// It keeps UGC defaults (no scripts) and enables safe inline CSS via style attributes.
func newUGCPolicy() *bluemonday.Policy {
	p := bluemonday.UGCPolicy()
	p.AllowStyles(allowedCSSProperties...).Globally()
	return p
}

// HTML sanitizes s using the post-content policy so that scripts and other
// unsafe markup are removed while safe HTML and allowed inline CSS remain.
// s is the raw HTML string submitted or stored for a post.
